#include "project/server/http/http_codec.h"
...
std::optional<Request> HttpCodec::parse(std::string_view raw_req) 
{
    ...
    if (req.method == Method::UNKNOWN) {
        LOG_WARN("Incorrectly specified method <UNKNOWN>", {
            field("raw_target", std::string(targets))
        });
        return std::nullopt;
    }
    ...
    LOG_TRACE("Target line parsed", {
        field("method", to_string(req.method)),
        field("path", std::string(req.path))
    });
    ...
    LOG_TRACE("Headers parsed", {
        field("header_count", req.headers.size())
    });
    ...
    if (end_headers != std::string::npos) {
        ...
        try {
            ...
            LOG_WARN("Failed to parse JSON body", {
                field("error", mess.what())
            });
            return std::nullopt;
        }
    }

    req.id = uni::common::uuid::generate_uuid_v4();
...
std::string HttpCodec::serialize(Response& resp) noexcept 
{
    LOG_TRACE("Serializing response", {
        field("status", uni::common::to_string(resp.status))
    });

    std::string body = resp.body.dump(4);
    std::string out = fmt::format("HTTP/1.1 {} {}\r\nContent-Type: application/json\r\nContent-Length: {}\r\n", 
        uni::common::to_string(resp.status), obsolete_reason(resp.status), body.size());
...
std::string HttpCodec::process(std::string_view raw_req) {
    ...
    if (!parse_w(raw_req)) {
        ...
        resp = makeResp(status::retCode::InvalidJsonOrParams);
    }
    else {
        resp = routing::RouterManager::Route(req_);

        if (!resp) {
            LOG_CRIT("Router not initialized", {
                field("method", to_string(req_.method)),
                field("path", req_.path)
            });
            resp = makeResp(status::retCode::InternalError);
        }
    }
