...
{
    status::log set_level = LogSystem::GetLevel();
...
{
    status::http s = status::to_http_status(code);
    std::vector<LogField> details_base = {
        field("retCode", code),
        field("retMesg", status::retMesg(code)),
        field("status", s),
        field("obsolete_reason", status::obsolete_reason(s))
    };
    details.insert(details.end(), details_base.begin(), details_base.end());

    linfo_ = LogInfo(to_log(code), msg, std::move(details), std::move(loc));
}
...
) : logger_(logger)
{
    std::vector<LogField> details_base = {
        field("status", status),
        field("obsolete_reason", status::obsolete_reason(status))
    };
    details.insert(details.end(), details_base.begin(), details_base.end());

    linfo_ = LogInfo(to_log(status), msg, std::move(details), std::move(loc));
}
...
bool LogSystem::Init(std::filesystem::path path)
{
    ...
    try {
        ...
        cw_ = std::make_unique<config::ConfigWatcher>(
            path, 
            [](std::filesystem::path p) { return lm_->Init(p); }
        );
    } catch(const std::bad_alloc& msg) {
        sys_error("Error initializing LogManager and config::ConfigWatcher: {}", msg.what());
        return false;
    ...
    sys_info("LogManager and config::ConfigWatcher initialized");
    return true;
}
...
status::log LogSystem::GetLevel() noexcept
{
    if (!lm_) {
